...
#: 可累计进标题区间的内容块类型；代码块单独处理，hr/frontmatter 无内容。
_CONTENT_KINDS = frozenset({"paragraph", "list", "table", "quote", "image"})
...
def _flush_region(
    ...
    if content or code:
        regions.append(_Region(chain=tuple(chain), content=tuple(content), code=tuple(code)))
    content.clear()
    code.clear()
